...
pub struct ParsedSegment {
    pub video_frames: Vec<RawFrame>,
    pub audio_frames: Vec<RawFrame>,
}
...
impl SegmentParser {
    ...
    fn extract_frames_from_mdat(
        ...
        mdat_data: &[u8],
        moof: &mp4_atom::Moof,
        video_frames: &mut Vec<RawFrame>,
        audio_frames: &mut Vec<RawFrame>
    ) -> anyhow::Result<()> {
    ...
    fn extract_video_nalus(&self, sample: &[u8]) -> anyhow::Result<Vec<u8>> {
    ...
    fn extract_aac_frame(&self, sample: &[u8]) -> anyhow::Result<Vec<u8>> {
